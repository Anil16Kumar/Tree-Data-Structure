package problem.Maximum_In_Every_Level;
/*
Given a binary tree, find the largest value in each level.

Example 1:

Input :
        1
       / \
      2   3

Output : 1 3
Explanation :
There are two levels in the tree :
1. {1}, max = 1
2. {2, 3}, max = 3
Example 2:

Input :
        4
       / \
      9   2
     / \   \
    3   5   7

Output : 4 9 7
Explanation :
There are three levels in the tree:
1. {4}, max = 4
2. {9, 2}, max = 9
3. {3, 5, 7}, max=7
Your task :
You don't need to read input or print anything. Your task is to complete the function largestValues() which takes the root node of the tree as input and returns a vector containing the largest value in each level.

Expected Time Complexity : O(n) , where n = number of nodes
Expected Auxiliary Space : O(n) , where n = number of nodes

Constraints :
1 ≤ Number of nodes ≤ 10^5
* */
import java.util.*;

public class MainActivity {
    public static void main(String[] args) {
        LevelTreeNode node=new LevelTreeNode();

        node.addData(new Scanner(System.in));
        node.diplay();
        node.allPathFromRootToLeaf();
        node.levelOrder();
        System.out.println("-->"+largestValuesInEveryLevel(node.root));
    }

    public static ArrayList<Integer> largestValuesInEveryLevel(LevelTreeNode.Node root){
        ArrayList<Integer> answer=new ArrayList<>();
        maxInEveryLevel(root, answer);
        return answer;
    }
    private static void maxInEveryLevel(LevelTreeNode.Node node, ArrayList<Integer> answer){
        if(node==null)
            return;

        Queue<LevelTreeNode.Node> que=new LinkedList<>();
        que.add(node);
        while(!que.isEmpty()){
            int size=que.size();
            int temp=Integer.MIN_VALUE;
            for(int i=0;i<size;i++){
                LevelTreeNode.Node curr=que.poll();
                if(curr.data>temp)
                    temp=curr.data;

                if(curr.left!=null) que.add(curr.left);
                if(curr.right!=null) que.add(curr.right);
            }
            answer.add(temp);
        }
    }
}
